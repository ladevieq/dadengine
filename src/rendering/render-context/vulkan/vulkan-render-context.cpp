...
#include "vulkan-vertex-buffer.hpp"

namespace DadEngine
{
    VulkanRenderContext::VulkanRenderContext(Window&_InWindow)
    {
        ...
        m_uiGraphicsQueueFamilyIndex =
            VulkanHelper::CheckDeviceQueueFamilyProperties(m_PhysicalDevice, VK_QUEUE_GRAPHICS_BIT);
    ...
    void VulkanRenderContext::ClearColorBuffer(Color &_InClearColor, CommandBuffer *_InCommandBuffer)
    {
        ...
        image_memory_barrier_from_color_to_clear.dstAccessMask =
        ...
        image_memory_barrier_from_color_to_clear.oldLayout =
            VK_IMAGE_LAYOUT_UNDEFINED; // m_BackBuffer->m_CurrentLayout; // Swapchain image is supposed to be undefined after being acquired
        ...
        vkCmdPipelineBarrier(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                             ...
                             NULL, 0, NULL, 1, &image_memory_barrier_from_color_to_clear);

        vkCmdClearColorImage(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                             m_BackBuffer->m_Image, m_BackBuffer->m_CurrentLayout, &color, 1U,
                             &m_BackBuffer->m_ImageSubresourceRange);
    }
    ...
    void VulkanRenderContext::ClearDepthStencilBuffer(float _InDepthValue, uint32_t _InStencilValue, CommandBuffer *_InCommandBuffer)
    {
        ...
        vkCmdPipelineBarrier(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                             VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0,
                             NULL, 0, NULL, 1, &image_memory_barrier_from_depth_to_clear);

        m_DepthStencilBuffer.m_CurrentLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;


        vkCmdClearDepthStencilImage(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                                    m_DepthStencilBuffer.m_Image, m_DepthStencilBuffer.m_CurrentLayout,
                                    &depthStencil, 1U, &m_DepthStencilBuffer.m_ImageSubresourceRange);
    }
    ...
    void VulkanRenderContext::Present(CommandBuffer *_InCommandBuffer)
    {
        ...
        vkCmdPipelineBarrier(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                             ...
                             &image_memory_barrier_from_anything_to_present);
    }
    ...
    void VulkanRenderContext::Draw(VertexBuffer *_InVertexBuffer, CommandBuffer *_InCommandBuffer)
    {
        vkCmdDraw(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                  _InVertexBuffer->m_uiVertexCount, 1U, 0U, 0U);
    }
    ...
    void VulkanRenderContext::BindVertexBuffer(VertexBuffer *_InVertexBuffer, CommandBuffer *_InCommandBuffer)
    {
        ...
        vkCmdBindVertexBuffers(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer, 0U, 1U,
                               &((VulkanVertexBuffer *)_InVertexBuffer)->m_Buffer, &deviceSize);
    }

    void VulkanRenderContext::BindShaderProgram(Shader *_InShader, CommandBuffer *_InCommandBuffer)
    {
        vkCmdBindPipeline(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                          VK_PIPELINE_BIND_POINT_GRAPHICS, ((VulkanShader *)_InShader)->m_GraphicsPipeline);

        ((VulkanShader *)_InShader)->UpdateUBO();

        // Also bind shaders params ?
        vkCmdBindDescriptorSets(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
                                VK_PIPELINE_BIND_POINT_GRAPHICS, ((VulkanShader *)_InShader)->m_PipelineLayout,
                                0U, 1U, &((VulkanShader *)_InShader)->m_DescriptorSet, 0U, VK_NULL_HANDLE);
    }

    void VulkanRenderContext::BeginRenderPass(RenderPass *_InRenderPass,
                                              Framebuffer *_InFrameBuffer,
                                              CommandBuffer *_InCommandBuffer)
    {
        ...
        vkCmdBeginRenderPass(((VulkanCommandBuffer *)_InCommandBuffer)->m_CmdBuffer,
    ...
    {
        VkPipelineStageFlags wait_pipelinestage_flag =
            /*VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;*/ VK_PIPELINE_STAGE_TRANSFER_BIT;
    ...
    {
        return new VulkanVertexBuffer(_InVertexCount, _InVertexLayout, _InData, _InVerticesStride,
                                      m_Device, m_PhysicalDevice);
    }

    VertexShader *VulkanRenderContext::CreateVertexShader(const char *_InShaderCode,
    ...
    Shader *VulkanRenderContext::CreateShader(VertexShader *_InVertexShader,
                                              GeometryShader *_InGeometryShader,
                                              FragmentShader *_InFragmentShader,
                                              RenderPass *_InRenderpass)
    {
        return new VulkanShader(_InVertexShader, _InGeometryShader, _InFragmentShader, m_Device,
                                m_PhysicalDevice, ((VulkanRenderPass *)_InRenderpass)->m_Renderpass,
                                m_PipelineCache, *m_Swapchain, m_UBODescriptorPool);
    }
    ...
    Framebuffer *VulkanRenderContext::CreateFramebuffer(Extent2D &_InFramebufferSize, RenderPass *_InRenderPass)
    {
        ...
        return new VulkanFramebuffer(m_Device, *((VkExtent2D *)&_InFramebufferSize),
                                     *(VulkanRenderPass *)_InRenderPass, views.GetData(), views.Size());
    }
    ...
    void VulkanRenderContext::CreateDevice()
    {
        ...
        device_queue_create_info.Add({ VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO, VK_NULL_HANDLE, NULL,
                                       m_uiGraphicsQueueFamilyIndex, 1U, queuePriorities.GetData() });

        if (m_uiGraphicsQueueFamilyIndex != m_uiPresentationsQueueFamilyIndex)
            device_queue_create_info.Add({ VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
                                           VK_NULL_HANDLE, NULL, m_uiPresentationsQueueFamilyIndex,
                                           1U, queuePriorities.GetData() });
    ...
    void VulkanRenderContext::CreateCommandPool()
    {
        ...
        VK_CHECK_RESULT(vkCreateCommandPool(m_Device, &graphics_command_pool_create_info,
                                            VK_NULL_HANDLE, &m_GraphicsCommandPool));
        ...
        VK_CHECK_RESULT(vkCreateCommandPool(m_Device, &presentation_command_pool_create_info,
                                            VK_NULL_HANDLE, &m_PresentationCommandPool));
    }
    ...
    void VulkanRenderContext::CreateDescriptorPool()
    {
        ...
        VK_CHECK_RESULT(vkCreateDescriptorPool(m_Device, &descriptor_pool_create_info,
                                               VK_NULL_HANDLE, &m_UBODescriptorPool));
    }
    ...
    void VulkanRenderContext::CreatePresentationSurface(PlatformWindow &_InWindow)
    {
        ...
        VK_CHECK_RESULT(vkCreateWin32SurfaceKHR(m_Instance, &win32_surface_create_info,
                                                VK_NULL_HANDLE, &m_PresentationSurface));
    }
    ...
    void VulkanRenderContext::CreateDepthStencilBuffer()
    {
        ...
        m_DepthStencilBuffer =
            VulkanDepthStencilBuffer(m_Device, format, m_Swapchain->m_SwapchainExtent, m_PhysicalDevice,
                                     VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
                                     m_SetupCommandBuffer);
    }
    ...
    void VulkanRenderContext::CreateSemaphores()
    {
        ...
        VK_CHECK_RESULT(vkCreateSemaphore(m_Device, &semaphore_create_info, VK_NULL_HANDLE,
